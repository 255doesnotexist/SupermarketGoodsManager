...
std::string Base64::encode(const std::string& source_string){
    return encode(source_string.c_str(),source_string.size());
}

std::string Base64::encode(const int& source_integer){
    std::ostringstream os; os<<source_integer;
    return encode(os.str().c_str(),os.str().size());
}

std::string Base64::encode(const double& source_double){
    std::ostringstream os; os<<source_double;
    return encode(os.str().c_str(),os.str().size());
}
